package elementicraft.common.MCACommonLibrary.animation;
...
import elementicraft.common.MCACommonLibrary.math.Vector3f;
...
public abstract class AnimationHandler {
	 ...
	 */
	@SideOnly(Side.CLIENT)
	public static void performAnimationInModel(HashMap<String, MCAModelRenderer> parts, EntityLiving entity)
	{
		for (Map.Entry<String, MCAModelRenderer> entry : parts.entrySet()) {
			...
			for(Channel channel : ((IMCAnimatedEntity) entity).getAnimationHandler().animCurrentChannels)
			{
				if(channel.mode != Channel.CUSTOM) {
					float currentFrame = ((IMCAnimatedEntity) entity).getAnimationHandler().animCurrentFrame.get(channel.name);

					//Rotations
					KeyFrame prevRotationKeyFrame = channel.getPreviousRotationKeyFrameForBox(boxName, ((IMCAnimatedEntity) entity).getAnimationHandler().animCurrentFrame.get(channel.name));
					int prevRotationKeyFramePosition = prevRotationKeyFrame != null ? channel.getKeyFramePosition(prevRotationKeyFrame) : 0;

					KeyFrame nextRotationKeyFrame = channel.getNextRotationKeyFrameForBox(boxName, ((IMCAnimatedEntity) entity).getAnimationHandler().animCurrentFrame.get(channel.name));
					...
					//Translations
					KeyFrame prevTranslationKeyFrame = channel.getPreviousTranslationKeyFrameForBox(boxName, ((IMCAnimatedEntity) entity).getAnimationHandler().animCurrentFrame.get(channel.name));
					int prevTranslationsKeyFramePosition = prevTranslationKeyFrame != null ? channel.getKeyFramePosition(prevTranslationKeyFrame) : 0;

					KeyFrame nextTranslationKeyFrame = channel.getNextTranslationKeyFrameForBox(boxName, ((IMCAnimatedEntity) entity).getAnimationHandler().animCurrentFrame.get(channel.name));
					...
					((CustomChannel)channel).update(parts, (IMCAnimatedEntity) entity);
				}
			}
